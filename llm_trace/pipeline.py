...
class LlmTracePipeline:
    ...
    def load_documents(self, demo: bool = False) -> list[ProcessDocument]:
        if demo:
            return [build_demo_document()]
        documents = self.parser.parse(self.config.get("data", {}))
        if documents:
            return documents
        return [ProcessDocument(document_id="empty_document", evidence=[])]
    ...
    def run_document(self, document: ProcessDocument) -> PipelineResult:
        ...
        entities, relations = self.extractor.extract(document.document_id, document.evidence)
